...
ISSUE_PATTERNS = [
    ...
    ("끝인사 누락",       r"끝인사\s*(누락|미이행|안\s*함)|마무리\s*인사\s*(누락|미이행)|다소\s*어색한\s*끝인사"),
    ("상황별인사 미준수", r"상황별\s*인사\s*(미준수|누락|오류)|상황에\s*맞(지\s*않는|는)\s*인사|처리불가.*인사|브랜드명이\s*아닌\s*상품명"),
    ("통화종료 미흡",     r"통화\s*종료\s*(미흡|누락|먼저)|먼저\s*끊|종료\s*(미흡|오류)|선종료멘트|3초\s*대기.*종료"),
    ...
    ("존칭 오류",         r"존칭\s*(오류|누락|미사용)|요조체|비\s*정중|반말|하세요체\s*오류|합쇼체\s*오류|셨을까요|존댓말|잘못된\s*존칭"),
    ("토막말/도치",       r"토막말|도치\s*(문장)?|문장\s*(불완전|끊김|짧음)|짧게\s*끊|끊어\s*말"),
    ("두괄식 미이행",     r"두괄식|결론\s*(먼저|후에|나중)|요점\s*먼저"),
    ("문법/조사오류",     r"띄어\s*쓰기|문법\s*(오류|틀림)|맞춤법|오타|철자\s*(오류|틀림)|조사\s*(미사용|오류|어색)|조사\s*미사용"),

    # ── 고객응대 태도 ──
    ("쿠션멘트/사과멘트 누락", r"쿠션\s*(멘트|어)\s*(누락|미사용|사용\s*지양)|사과\s*멘트\s*(누락|미사용)|안타깝게도|사과.*양해.*누락|양해.*사과.*누락|케어멘트\s*누락"),
    ("양해 누락",         r"양해\s*(누락|미이행|안\s*함)|양해\s*멘트\s*(누락|미이행)|대기\s*양해|보류\s*양해|보류\s*전\s*양해|선\s*양해\s*후\s*안내.*지켜지지"),
    ("대기 안내 누락",    r"대기\s*안내\s*(누락|미이행)|보류\s*안내\s*(누락|미이행)|홀드\s*안내\s*(누락|미이행)"),
    ("보류버튼 미사용",   r"보류\s*버튼\s*(미사용|누락|사용\s*안)|보류버튼\s*(미사용|누락)"),
    ...
    # ── 안내·업무 처리 ──
    ("임의안내",          r"임의\s*(안내|설명)|가이드\s*확인\s*없이|고객\s*요청\s*없이.*선안내|이전\s*상담사.*못드린\s*것\s*같다"),
    ...
    ("프로세스 미준수",   r"프로세스\s*(미준수|위반|누락|미이행)|절차\s*(누락|미준수|위반)|순서\s*(오류|위반)|약속\s*(미준수|불이행)"),
    ("처리 오류/지연",    r"처리\s*(오류|지연|미흡|실수)|업무\s*(오류|실수|지연)|잘못\s*처리"),

    # ── 이력·전산 ──
    ("문의유형 오선택",   r"문의유형|상담유형|중분류|대분류|소분류|상담상태\s*(오선택|미완료)|입고\s*사유.*반품\s*선택|교환.*반품.*선택|반품.*교환.*선택"),
    ("OMS/전산 오처리",   r"oms|미회수교환|전산\s*오처리|시스템\s*오처리"),
    ("수신여부 미확인",   r"수신\s*여부\s*(미확인|확인\s*누락|확인\s*안)"),
    ("이력 미기재",       r"이력\s*(누락|미기재|미작성|안\s*씀)|상담\s*이력\s*(누락|미기재)|메모\s*(누락|미작성)|기재\s*(누락|안\s*됨)|미기재|주소\s*미기재|수량\s*미기재|상담이력\s*미기재"),
    ("이력 내용 미흡",    r"세부\s*내용\s*(기재\s*누락|미흡)|내용\s*기재\s*미흡|상세기재\s*필요|이력.*기재됨(?!.*오)|주소.*기재.*누락(?!.*오)|기재.*누락.*내용|내용.*기재.*누락|수량.*기재.*누락(?!.*오)"),
    ("오기재/잘못기재",   r"오\s*기재|잘못\s*기재|기재\s*(오류|실수|잘못)|이력\s*(오기재|잘못\s*기재|오류|틀림)|메모\s*(오류|잘못|틀림)|잘못\s*입력|입력\s*오류(?!.*전산)|미수긍으로\s*기재|정상품으로.*기재|미확인으로\s*기재|컨시어지.*기재됨"),
    ("관리자메모 누락",   r"관리자\s*메모|관리자메모|첫줄\s*기재\s*누락"),
    ("전산처리 오류",     r"전산\s*(오류|누락|미처리|실수)|시스템\s*(오류|누락)|입력\s*(오류|누락|실수)"),

    # ── 선택 오류 ──
    ("오선택/선택오류",   r"오\s*선택|잘못\s*선택|선택\s*(오류|실수|잘못)|선택지\s*(오류|잘못)|항목\s*선택\s*(오류|잘못)|메뉴\s*(오선택|잘못\s*선택)|카테고리\s*(오류|잘못)"),

    # ── 채팅/게시판 전용 ──
    ("가독성 부족",       r"가독성|문단\s*(나눔|없음|미흡)|엔터\s*(없음|미사용)|줄\s*바꿈\s*(없음|미흡)|단락\s*(없음|미구분)"),
    ("첨부 누락",         r"(사진|파일|첨부|이미지)\s*(누락|미첨부|빠짐)"),
    ("민원체크 누락",     r"민원\s*체크\s*(누락|미이행)|민원체크|상담상태\s*미완료"),

    # ── 음성 전용 ──
    ("음성숙련도 미흡",   r"음성\s*숙련(도)?\s*(미흡|부족)|발음\s*(불명확|미흡|나쁨|불분명)|말\s*속도\s*(빠름|느림|미흡)|목소리\s*(미흡|작음|불명확)"),
    ("복창 미흡",         r"복창\s*(누락|미흡|일부만)|배송지.*복창|일부만\s*복창"),
]

ROOT_CAUSE = {
    "첫인사 누락":           "첫인사 체크리스트 미이행 → 콜 시작 루틴 점검",
    "끝인사 누락":           "끝인사 체크리스트 미이행 → 콜 마무리 루틴 점검",
    "상황별인사 미준수":     "상황 맥락 파악 부족 → 상황별 인사 멘트 유형 교육",
    ...
    "문법/조사오류":         "문서·발화 작성 능력 부족 → 글쓰기·말하기 교육",
    "쿠션멘트/사과멘트 누락":"공감 표현 부족 → 쿠션멘트 패턴 암기 훈련",
    "양해 누락":             "고객 공감 부족 → 양해 멘트 패턴 훈련",
    "대기 안내 누락":        "대기 안내 절차 누락 → 보류 전 멘트 습관화",
    "보류버튼 미사용":       "보류 절차 미숙지 → 10초 기준 보류버튼 사용 교육",
    "호응 부족":             "경청 자세 부족 → 적극적 호응 연습",
    "감정연출 미흡":         "감성 응대 부족 → 감정연출 스크립트 교육",
    "임의안내":              "업무 매뉴얼 이해 부족 → 안내 전 가이드 확인 습관화",
    ...
    "문의유형 오선택":       "문의유형 분류 기준 미숙지 → 유형 분류 기준표 재교육",
    "OMS/전산 오처리":       "OMS 시스템 활용 미숙 → 처리 유형별 OMS 입력 재교육",
    "수신여부 미확인":       "문자발송 후 확인 절차 누락 → 발송 후 수신확인 루틴화",
    "이력 미기재":           "기록 습관 부재 → 상담 후 이력 루틴화",
    "이력 내용 미흡":        "이력 작성 기준 미숙지 → 세부 기재 항목 체크리스트 교육",
    "오기재/잘못기재":       "이력 작성 부주의 → 기재 내용 재확인 습관화 교육",
    "관리자메모 누락":       "관리자메모 작성 기준 미숙지 → 첫줄 기재 규정 재교육",
    "전산처리 오류":         "시스템 활용 미숙 → 전산 처리 재교육",
    "오선택/선택오류":       "메뉴/카테고리 선택 실수 → 선택 항목 재확인 습관화 교육",
    "가독성 부족":           "채팅 문서화 부족 → 구조화 글쓰기 교육",
    "첨부 누락":             "체크리스트 미활용 → 처리 단계 확인 습관화",
    "민원체크 누락":         "민원 등록 절차 미숙지 → 민원체크·상담상태 설정 교육",
    "음성숙련도 미흡":       "발화 기술 부족 → 음성 훈련 및 모니터링",
    "복창 미흡":             "정보 확인 절차 부족 → 배송지 복창 루틴화 교육",
    "오기재/잘못기재":       "이력 작성 부주의 → 기재 내용 재확인 습관화 교육",
...
def crisis_analysis():
    ...
    for agent in df_all["상담사"].unique():
        ...
        # ── 트렌드: 2차수 이상이면 첫→끝 전체 변화폭, 1차수면 유지
        trend = "유지"; trend_val = 0.0
        if len(period_avgs) >= 3:
            # 선형회귀 기울기로 판단 (노이즈 방지)
            xs = list(range(len(period_avgs)))
            n = len(xs)
            mx_x = sum(xs)/n; mx_y = sum(period_avgs)/n
            slope = sum((x-mx_x)*(y-mx_y) for x,y in zip(xs,period_avgs)) / max(sum((x-mx_x)**2 for x in xs),1)
            trend_val = round(slope, 1)
            if slope <= -1.5: trend = "악화"
            elif slope >= 1.5: trend = "개선"
        elif len(period_avgs) == 2:
            trend_val = round(period_avgs[-1] - period_avgs[0], 1)
            if trend_val <= -3: trend = "악화"
            elif trend_val >= 3: trend = "개선"

        # ── 최대 하락 항목 탐색 ──
        worst_item = ""; worst_drop = 0.0
        if len(periods_sorted) >= 2:
            first_p, last_p = periods_sorted[0], periods_sorted[-1]
            for ch_df2, items_map in [(df_c, CALL_ITEMS),(df_h, CHAT_ITEMS),(df_b, BOARD_ITEMS)]:
                if ch_df2 is None or len(ch_df2)==0: continue
                a_ch = flt(ch_df2[ch_df2["상담사"]==agent])
                for iname,(ci,mx) in items_map.items():
                    sc_col = f"{iname}_점수"
                    if sc_col not in a_ch.columns: continue
                    s_first = a_ch[a_ch["평가기간"]==first_p][sc_col].dropna().tolist()
                    s_last  = a_ch[a_ch["평가기간"]==last_p][sc_col].dropna().tolist()
                    r_first = comp_rate(s_first,[mx]*len(s_first))
                    r_last  = comp_rate(s_last,[mx]*len(s_last))
                    if r_first is not None and r_last is not None:
                        drop = r_first - r_last
                        if drop > worst_drop:
                            worst_drop = drop; worst_item = iname
        ...
        results.append({
            ...
            "평가건수": len(adf_f), "트렌드": trend, "트렌드값": trend_val,
            "반복이슈수": len(repeat), "최다이슈": top_issue[0],
            "최다이슈횟수": top_issue[1], "채널별점수": ch_scores, "이슈카운터": ic,
            "최대하락항목": worst_item, "최대하락폭": round(worst_drop,1),
            "평가차수": len(periods_sorted),
        })
        ...
        with col_a:
            ...
            if len(danger):
                for _, r in danger.iterrows():
                    ...
                    trend_lbl = f"{'📉 점점 악화' if r['트렌드']=='악화' else '📊 유지'} ({r['트렌드값']:+.1f}점/차수)"
                    worst_txt = f" · 최대하락: [{r['최대하락항목']}] {r['최대하락폭']:.1f}%p↓" if r["최대하락항목"] else ""
                    st.markdown(f'''<div class="alert-card">
                        <div class="alert-title">🚨 즉시 코칭 필요 — {r["상담사"]}</div>
                        <div class="alert-body">
                            평균 {r["전체평균"]}점 · {trend_lbl}{worst_txt}<br>
            ...
            if len(warning):
                for _, r in warning.iterrows():
                    ...
                    st.markdown(f'''<div class="warn-card">
                        ...
                        <div class="warn-body">반복이슈 {r["반복이슈수"]}개 · 최다: {r["최다이슈"]}{worst_txt}</div>
                    </div>''', unsafe_allow_html=True)
            if len(good):
                for _, r in good.iterrows():
                    trend_lbl = f"📈 점점 개선 ({r['트렌드값']:+.1f}점/차수)"
                    st.markdown(f'''<div class="good-card">
                        <div class="good-title">✅ 개선 중 — {r["상담사"]}</div>
                        <div class="good-body">{trend_lbl} · 평균 {r["전체평균"]}점</div>
                    </div>''', unsafe_allow_html=True)
